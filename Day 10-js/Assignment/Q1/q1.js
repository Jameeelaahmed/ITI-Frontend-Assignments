...
let name= prompt("Enter your FULL name");//ja
let mail= prompt("Enter your email address");

let validName=/\S[a-zA-z]{3,}\s[a-zA-z]{3,}(\s[a-zA-z]{3,})*\S$/ig;

let testName=validName.test(name);
// console.log(validName.test(name)+'from out of the loop');

let validMail=/\w+@\w+(.com|.net|.edu|.org).eg$/ig;

console.log(mail.match(validMail));
